const WallpaperLib = {
    ...
    setup(containerSelector, options = {}) {
        ...
        container.style.setProperty('--dot-gap', `${dotGap}px`);
        container.style.setProperty('--header-font', `${(10 * finalScale).toFixed(1)}px`);
        container.style.setProperty('--week-font', `${(7 * finalScale).toFixed(1)}px`);

        const statsBaseSize = params.res === '8k' ? 16.2 : 14.4;
        container.style.setProperty('--stats-font', `${(statsBaseSize * finalScale).toFixed(1)}px`);

        // 4. Handle Stats visibility
        const statsElement = document.getElementById('stats');
        if (statsElement) {
            statsElement.style.fontSize = `var(--stats-font, ${(14 * finalScale).toFixed(1)}px)`;
        }
    ...
    renderControls() {
        ...
        document.getElementById('apply-btn').onclick = () => {
            ...
            if (res && res !== 'custom') {
                ...
                url.searchParams.set('w', document.getElementById('custom-w').value);
                url.searchParams.set('h', document.getElementById('custom-h').value);
            } else {
                url.searchParams.delete('res');
                url.searchParams.delete('w');
                url.searchParams.delete('h');
